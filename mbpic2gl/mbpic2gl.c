/*--------------------------------------------------------------------*/
/* Skeleton 2-1/2D Electromagnetic Gridless OpenMP PIC code */
...
/* initialize ion background */
   ...
   cmdpost2gl(part,qi,sctx,-qme,np,idimp,nx,ny,nxhm,nyhm,nxeh,nye);
...
/* deposit current with standard procedure: updates part, cue */
      ...
      if (relativity==1)
         cmrdjpost2gl(part,cue,sctx,qme,dth,ci,np,idimp,nx,ny,nxhm,nyhm,
                      nxeh,nye,ipbc);
      else
         cmdjpost2gl(part,cue,sctx,qme,dth,np,idimp,nx,ny,nxhm,nyhm,
                     nxeh,nye,ipbc);
      dtimer(&dtime,&itime,1);
      time = dtime;
      tdjpost += time;
...
/* deposit charge with standard procedure: updates qe */
      ...
      cmdpost2gl(part,qe,sctx,qme,np,idimp,nx,ny,nxhm,nyhm,nxeh,nye);
      for (j = 0; j < nxeh*nye; j++) {
         qe[j] += qi[j];
      }
      dtimer(&dtime,&itime,1);
...
/* deposit time derivative of current: updates dcu */
         if (relativity==1)
            cmrd2jpost2gl(part,dcu,sctx,qme,ci,np,idimp,nx,ny,nxhm,nyhm,
                          nxeh,nye);
         else
            cmd2jpost2gl(part,dcu,sctx,qme,np,idimp,nx,ny,nxhm,nyhm,
                         nxeh,nye);
...
/*       }                                     */
/*       if (relativity==1) */
/*          cmrdsjpost2gl(part,cue,dcu,sctx,qme,ci,np,idimp,nx,ny, */
/*                        nxhm,nyhm,nxeh,nye);                     */
/*       else */
/*          cmdsjpost2gl(part,cue,dcu,sctx,qme,ci,np,idimp,nx,ny,  */
/*                       nxhm,nyhm,nxeh,nye);                      */
/*       ccuperp2gl(cue,nx,ny,nxhm,nyhm,nxeh,nye); */
...
/* analytic Boris mover, gamma constant during time step */
         cmarbpush23gl(part,fxyze,bxyze,qbme,dt,dth,ci,&wke,idimp,np,
                       nx,ny,nxhm,nyhm,nxeh,nye,ipbc);
/* exact analytic Boris mover, gamma varies during time step */
/*       cmearbpush23gl(part,fxyze,bxyze,qbme,dt,dth,ci,&wke,idimp,np, */
/*                      nx,ny,nxhm,nyhm,nxeh,nye,ipbc);                */
/* analytic non-relativistic Boris mover */
      else
         cmabpush23gl(part,fxyze,bxyze,qbme,dt,dth,&wke,idimp,np,nx,
