...
! initialize ion background
      qi = 0.0
      call MDPOST2GL(part,qi,sctx,-qme,np,idimp,nx,ny,nxhm,nyhm,nxeh,   &
     &nye)
!
...
! deposit current with standard procedure: updates part, cue
      ...
      if (relativity==1) then
         call MRDJPOST2GL(part,cue,sctx,qme,dth,ci,np,idimp,nx,ny,nxhm, &
     &nyhm,nxeh,nye,ipbc)
      else
         call MDJPOST2GL(part,cue,sctx,qme,dth,np,idimp,nx,ny,nxhm,nyhm,&
     &nxeh,nye,ipbc)
      endif
      call dtimer(dtime,itime,1)
      time = real(dtime)
...
! deposit time derivative of current: updates dcu
         if (relativity==1) then
            call MRD2JPOST2GL(part,dcu,sctx,qme,ci,np,idimp,nx,ny,nxhm, &
     &nyhm,nxeh,nye)
         else
            call MD2JPOST2GL(part,dcu,sctx,qme,np,idimp,nx,ny,nxhm,nyhm,&
     &nxeh,nye)
         endif
...
!        cue = cmplx(0.0,0.0)
!        if (relativity==1) then
!           call MRDSJPOST2GL(part,cue,dcu,sctx,qme,ci,np,idimp,nx,ny,  &
!    &nxhm,nyhm,nxeh,nye)
!        else
!           call MDSJPOST2GL(part,cue,dcu,sctx,qme,ci,np,idimp,nx,ny,   &
!    &nxhm,nyhm,nxeh,nye)
!        endif
!        call CUPERP2GL(cue,nx,ny,nxhm,nyhm,nxeh,nye)
...
! analytic Boris mover, gamma constant during time step
         call MARBPUSH23GL(part,fxyze,bxyze,qbme,dt,dth,ci,wke,idimp,np,&
     &nx,ny,nxhm,nyhm,nxeh,nye,ipbc)
! exact analytic Boris mover, gamma varies during time step
!        call MEARBPUSH23GL(part,fxyze,bxyze,qbme,dt,dth,ci,wke,idimp,np&
!    &,nx,ny,nxhm,nyhm,nxeh,nye,ipbc)
! analytic non-relativistic Boris mover
      else
         call MABPUSH23GL(part,fxyze,bxyze,qbme,dt,dth,wke,idimp,np,nx, &
