c Fortran Library for Skeleton 2-1/2D Electromagnetic Gridless OpenMP 
c PIC Code
...
c-----------------------------------------------------------------------
      subroutine MBPUSH23GL(part,fxy,bxy,qbm,dt,dtc,ek,idimp,nop,nx,ny, 
     1nxhm,nyhm,nxvh,nyv,ipbc)
...
c 60*nxhm*nyhm + 57 flops/particle, 1 divide, 3*NX*NY + 5 loads,
c 5 stores
...
!$OMP& fim,exl,eyl,ezl,bxl,byl,bzl,ex,ey,ez,bx,by,bz,dx,dy,dz,ox,oy,oz, 
!$OMP& acx,acy,acz,omxt,omyt,omzt,omt,anorm,rot1,rot2,rot3,rot4,rot5,   
!$OMP& rot6,rot7,rot8,rot9,zt1,zt2,zt3,zt4,ssctx) REDUCTION(+:sum1)
      do 50 i = 1, nop
      x = part(1,i)
      y = part(2,i)
...
!     ezl = ezl + (real(fxy(3,j,k)*zt3) + real(fxy(3,j,k1)*zt4))
      ...
      frm = real(zt4)
      fim = aimag(zt4)
      ezl = ezl + (frp*zr3 - fip*zi3 + frm*zr4 - fim*zi4)
!     bxl = bxl + (real(bxy(1,j,k)*zt3) + real(bxy(1,j,k1)*zt4))
      ...
      frm = real(zt4)
      fim = aimag(zt4)
      bxl = bxl + (frp*zr3 - fip*zi3 + frm*zr4 - fim*zi4)
!     byl = byl + (real(bxy(2,j,k)*zt3) + real(bxy(2,j,k1)*zt4))
      ...
      frm = real(zt4)
      fim = aimag(zt4)
      byl = byl + (frp*zr3 - fip*zi3 + frm*zr4 - fim*zi4)
!     bzl = bzl + (real(bxy(3,j,k)*zt3) + real(bxy(3,j,k1)*zt4))
      ...
      frm = real(zt4)
      fim = aimag(zt4)
      bzl = bzl + (frp*zr3 - fip*zi3 + frm*zr4 - fim*zi4)
   20 continue
c mode number kx = 0
...
c-----------------------------------------------------------------------
      subroutine MRBPUSH23GL(part,fxy,bxy,qbm,dt,dtc,ci,ek,idimp,nop,nx,
...
c OpenMP version, parallelization over particles
c 60*nxhm*nyhm + 69 flops/particle, 4 divides, 2 sqrts,
...
!$OMP& fim,exl,eyl,ezl,bxl,byl,bzl,ex,ey,ez,bx,by,bz,dx,dy,dz,ox,oy,oz, 
!$OMP& acx,acy,acz,omxt,omyt,omzt,omt,anorm,rot1,rot2,rot3,rot4,rot5,   
!$OMP& rot6,rot7,rot8,rot9,p2,gami,qtmg,dtg,zt1,zt2,zt3,zt4,ssctx)      
!$OMP& REDUCTION(+:sum1)
      do 50 i = 1, nop
      x = part(1,i)
      y = part(2,i)
...
c-----------------------------------------------------------------------
      subroutine MABPUSH23GL(part,fxy,bxy,qbm,dt,dtc,ek,idimp,nop,nx,ny,
     1nxhm,nyhm,nxvh,nyv,ipbc)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time using gridless spectral
c version, with magnetic field, periodic boundaries and various particle
...
!$OMP& PRIVATE(i,j,k,k1,x,y,dkx,dky,at1,at3,zr3,zi3,zr4,zi4,frp,fip,frm,
!$OMP& fim,exl,eyl,ezl,bxl,byl,bzl,ex,ey,ez,bx,by,bz,dx,dy,dz,ox,oy,oz, 
!$OMP& acx,acy,acz,omxt,omyt,omzt,qtmh,omt,omti,epl,anorm,rot1,rot2,    
!$OMP& rot3,rot4,rot5,rot6,rot7,rot8,rot9,zt1,zt2,zt3,zt4,ssctx)             
!$OMP& REDUCTION(+:sum1)
      do 50 i = 1, nop
      x = part(1,i)
      y = part(2,i)
...
c-----------------------------------------------------------------------
      subroutine MARBPUSH23GL(part,fxy,bxy,qbm,dt,dtc,ci,ek,idimp,nop,nx
...
!$OMP& PRIVATE(i,j,k,k1,x,y,dkx,dky,at1,at3,zr3,zi3,zr4,zi4,frp,fip,frm,
!$OMP& fim,exl,eyl,ezl,bxl,byl,bzl,ex,ey,ez,bx,by,bz,dx,dy,dz,ox,oy,oz, 
!$OMP& px,py,pz,p2,gam0,wk,ug0,u2g0,acx,acy,acz,omt,omti,epl,omxt,omyt, 
!$OMP& omzt,ugt0,u2gt0,gami,tau,f,fp,qtmg,dtg,anorm,rot1,rot2,rot3,rot4,
!$OMP& rot5,rot6,rot7,rot8,rot9,zt1,zt2,zt3,zt4,ssctx) REDUCTION(+:sum1)
      do 50 i = 1, nop
      x = part(1,i)
      y = part(2,i)
c find electric and magnetic field
...
c-----------------------------------------------------------------------
      subroutine MEARBPUSH23GL(part,fxy,bxy,qbm,dt,dtc,ci,ek,idimp,nop, 
     1nx,ny,nxhm,nyhm,nxvh,nyv,ipbc)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time using gridless spectral
c version, for relativistic particles with magnetic field, periodic
c boundaries and various particle boundary conditions.
...
c loop over particles
!$OMP PARALLEL DO
!$OMP& PRIVATE(i,j,k,k1,ii,x,y,dkx,dky,at1,at3,zr3,zi3,zr4,zi4,frp,fip, 
!$OMP& frm,exl,eyl,ezl,bxl,byl,bzl,ex,ey,ez,bx,by,bz,dx,dy,dz,ox,oy,oz, 
!$OMP& tx,ty,tz,om2,omega,di,dt1,ep,e2,et,err,w2,wl2,al2,w,al,wi,ali,
!$OMP& wi2,ali2,om2t,ws,px,py,pz,up,ut,ud,p2,gam0,wk,dgp0,dgt0,d2gp0,
!$OMP& d2gt0,dg0,d2g0,wt2,ea,eb,ec,ed,fa,fb,fc,tau,cs,sn,csh,snh,fpp,fp,
!$OMP& ft,t2,t3,wt,wd,tn,tn2,tn2i,f,csc,snc,cshc,snhc,csd,snd,cshd,snhd,
!$OMP& gam,fpi,gami,erm,dtg,zt1,zt2,zt3,zt4,ssctx)
!$OMP& REDUCTION(+:sum1)
      do 70 i = 1, nop
      x = part(1,i)
      y = part(2,i)
...
c-----------------------------------------------------------------------
      subroutine MDJPOST2GL(part,cu,sctx,qm,dt,nop,idimp,nx,ny,nxhm,nyhm
     1,nxvh,nyv,ipbc)
...
c !$OMP END PARALLEL DO
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO PRIVATE(j,k,k1,dky,zt1,zt2,zt3,zt4)
      do 30 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k-1)*y
      zt1 = cmplx(cos(dky),-sin(dky))
...
c mode number ky = 0
      ...
      dky = dny*real(nyh)*y
      at1 = cos(dky)
c !$OMP PARALLEL DO PRIVATE(j,zt1,zt3)
      do 40 j = 2, nxhm
      zt3 = sctx(j-1)
...
c-----------------------------------------------------------------------
      subroutine MRDJPOST2GL(part,cu,sctx,qm,dt,ci,nop,idimp,nx,ny,nxhm,
     1nyhm,nxvh,nyv,ipbc)
c for 2-1/2d code, this subroutine calculates particle current density
...
c-----------------------------------------------------------------------
      subroutine MD2JPOST2GL(part,dcu,sctx,qm,nop,idimp,nx,ny,nxhm,nyhm,
     1nxvh,nyv)
...
c part(4,n) = y velocity of particle n
c part(5,n) = z velocity of particle n
c dcu(i,n,m) = time derivative of current density for fourier mode n,m
c sctx = scratch array for sines and cosines
c qm = charge on particle, in units of e
c nop = number of particles
c idimp = size of phase space = 5
...
c !$OMP END PARALLEL DO
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO PRIVATE(j,k,k1,dkx,dky,dkv,dp,dm,zt1,zt2,zt3,zt4)
      do 30 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k-1)
      dkv = dky*uy
...
c mode number ky = 0
      ...
      dky = dky*y
      at1 = cos(dky)
c !$OMP PARALLEL DO PRIVATE(j,dkx,dp,zt1,zt3)
      do 40 j = 2, nxhm
      dkx = dnx*real(j-1)
      dp = dkx*ux
      zt3 = sctx(j-1)
...
c-----------------------------------------------------------------------
      subroutine MRD2JPOST2GL(part,dcu,sctx,qm,ci,nop,idimp,nx,ny,nxhm, 
     1nyhm,nxvh,nyv)
c for 2-1/2d code, this subroutine calculates the time derivative of the
c particle current density for force-free particles,
c used to initialize the darwin tranverse electric field
c using gridless spectral version for relativistic particles,
c periodic boundaries, parallelization over fourier modes
c OpenMP version
...
c-----------------------------------------------------------------------
      subroutine MDSJPOST2GL(part,cu,dcu,sctx,qm,ci,nop,idimp,nx,ny,nxhm
     1,nyhm,nxvh,nyv)
...
c periodic boundaries
c Non-relativistic, do not use particles moving at the speed up light.
c OpenMP version, parallelization over fourier modes
...
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO
!$OMP& PRIVATE(j,k,k1,dkx,dky,dkv,dky2,dk2,dp,dm,sp,sm,zt1,zt2,zt3,zt4)
      do 30 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k-1)
      dky2 = dky*dky
...
c mode number ky = 0
      ...
      dky = dky*y
      at1 = cos(dky)
c !$OMP PARALLEL DO PRIVATE(j,dkx,dk2,dp,sp,zt1,zt3)
      do 40 j = 2, nxhm
      dkx = dnx*real(j-1)
      dk2 = dkx*dkx
      dp = dkx*ux
...
c c*Bn+1 = C*(c*Bn) - iS*(k X En) + iS*T*(k X JTn+1/2)
...
c cu(i,j,k) = transverse part of complex current density
...
c magnetic field energy is also calculated, using
c wm = nx*ny**sum((c2/affp)*|bxy(kx,ky)|**2), where c2 = 1./(ci*ci)
c nx/ny = system length in x/y direction
c nxhm/nyhm = number of fourier modes kept in x/y
c nxvh = first dimension of field arrays, must be >= nxhm
c nyv = second dimension of field arrays, must be >= 2*nyhm
...
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO
!$OMP& PRIVATE(j,k,k1,dky,dky2,dkx,aft,t2,t,s,c,afs,sc,zt1,zt2,zt3,zt4,  
!$OMP& zt5,zt6,zt7,zt8,zt9,ws,wp)
!$OMP& REDUCTION(+:swp,sws)
      do 20 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k - 1)
      dky2 = dky*dky
...
c-----------------------------------------------------------------------
      subroutine M1BPUSH23GL(part,fxy,bxy,sctx,qbm,dt,dtc,ek,idimp,nop, 
     1nx,ny,nxhm,nyhm,nxvh,nyv,ipbc)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time using gridless spectral
c version, with magnetic field, periodic boundaries and various particle
...
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO
!$OMP& PRIVATE(j,k,k1,dky,zr3,zi3,zr4,zi4,frp,fip,frm,fim,exl,eyl,ezl,  
!$OMP& bxl,byl,bzl,zt1,zt2,zt3,zt4)
!$OMP& REDUCTION(+:ex,ey,ez,bx,by,bz)
      do 30 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k-1)*y
      zt1 = cmplx(cos(dky),sin(dky))
...
c mode number ky = 0
      ...
      bzl = 0.0d0
c !$OMP PARALLEL DO PRIVATE(j,zt1,zt3)
c !$OMP& REDUCTION(+:exl,eyl,ezl,bxl,byl,bzl)
      do 40 j = 2, nxhm
      zt3 = sctx(j-1)
c special case to match conventional PIC code
      if (nyhm.eq.nyh) then
...
c-----------------------------------------------------------------------
      subroutine M1RBPUSH23GL(part,fxy,bxy,sctx,qbm,dt,dtc,ci,ek,idimp, 
...
c-----------------------------------------------------------------------
      subroutine MD4JPOST2GL(part,dcu,dcu2,sctx,qm,nop,idimp,nx,ny,nxhm,
     1nyhm,nxvh,nyv)
c for 2-1/2d code, this subroutine calculates the first two time
c derivatives of the particle current density for force-free particles,
c used to implement high accuracy maxwell solvers
c using gridless spectral version, periodic boundaries
c OpenMP version, parallelization over fourier modes
c 99*nxhm*nyhm flops per particle
c 24*nxhm*nyhm + 5 loads and stores
c plus (nxhm + nyhm) sines and cosines/particle
c input: all, output: dcu, dcu2, sctx
c first derivative of current density is calculated from the expression:
c dcu(i,n,m) = sum(qdi*exp(-sqrt(-1)*kn*x)*exp(-sqrt(-1)*km*y))
c where qdi = qm*vi*d, where i = x,y,z
c kn = 2*n*pi/nx, km = 2*m*pi/ny, k.vi = kn*vx+km*vy, d = -sqrt(-1)*k.vi
c second derivative of current density is calculated from the expression
c dcu2(i,n,m) = sum(qd2i*exp(-sqrt(-1)*kn*x)*exp(-sqrt(-1)*km*y))
c and qd2i = qm*vi*d2, d2 = -(k.vi)**2
c if nxhm=nx/2 and nyhm=ny/2, nx/2 and ny/2 modes are calculated, in a
c format compatible with the real to complex FFT used in traditional PIC
c part(1,n) = position x of particle n
c part(2,n) = position y of particle n
...
c part(4,n) = y velocity of particle n
c part(5,n) = z velocity of particle n
c dcu(i,n,m) = first time derivative of current density for fourier mode
c n,m
c dcu2(i,n,m) = second time derivative of current density for fourier
c mode n,m
c sctx = scratch array for sines and cosines
c qm = charge on particle, in units of e
c nop = number of particles
...
c nyv = second dimension of current array, must be >= 2*nyhm
      ...
      integer nop, idimp, nx, ny, nxhm, nyhm, nxvh, nyv
      real part, qm
      complex dcu, dcu2, sctx
      dimension part(idimp,nop), dcu(3,nxvh,nyv), dcu2(3,nxvh,nyv)
      dimension sctx(nxvh)
c local data
      integer i, j, k, k1, nxh, nyh, ny2
      real qmn, dnx, dny, dkx, dky, dkv, at1
...
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO 
!$OMP& PRIVATE(j,k,k1,dkx,dky,dkv,dp,dm,zt1,zt2,zt3,zt4,zt5,zt6)
      do 30 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k-1)
      dkv = dky*uy
      ...
      dm = dm - dkv
      zt3 = sctx(j-1)
c deposit first time derivative of current
      zt6 = zt2*zt3
      zt5 = zt1*zt3
      zt4 = zt6*cmplx(0.0,-dm)
      zt3 = zt5*cmplx(0.0,-dp)
      dcu(1,j,k) = dcu(1,j,k) + vx*zt3
      dcu(2,j,k) = dcu(2,j,k) + vy*zt3
      dcu(3,j,k) = dcu(3,j,k) + vz*zt3
      dcu(1,j,k1) = dcu(1,j,k1) + vx*zt4
      dcu(2,j,k1) = dcu(2,j,k1) + vy*zt4
      dcu(3,j,k1) = dcu(3,j,k1) + vz*zt4
c deposit second time derivative of current
      zt6 = zt6*dm*dm
      zt5 = zt5*dp*dp
      dcu2(1,j,k) = dcu2(1,j,k) - vx*zt5
      dcu2(2,j,k) = dcu2(2,j,k) - vy*zt5
      dcu2(3,j,k) = dcu2(3,j,k) - vz*zt5
      dcu2(1,j,k1) = dcu2(1,j,k1) - vx*zt6
      dcu2(2,j,k1) = dcu2(2,j,k1) - vy*zt6
      dcu2(3,j,k1) = dcu2(3,j,k1) - vz*zt6
   20 continue
c mode number kx = 0
c deposit first time derivative of current
      dp = dkv
      zt3 = zt1*cmplx(0.0,-dp)
      dcu(1,1,k) = dcu(1,1,k) + vx*zt3
      dcu(2,1,k) = dcu(2,1,k) + vy*zt3
      dcu(3,1,k) = dcu(3,1,k) + vz*zt3
c deposit second time derivative of current
      zt1 = zt1*dp*dp
      dcu2(1,1,k) = dcu2(1,1,k) - vx*zt1
      dcu2(2,1,k) = dcu2(2,1,k) - vy*zt1
      dcu2(3,1,k) = dcu2(3,1,k) - vz*zt1
c special case to match conventional PIC code
      if (nxhm.eq.nxh) then
         dkx = dnx*real(nxh)
         dm = dkx*ux - dkv
         zt4 = zt2*real(sctx(nxhm))
         zt2 = zt4*cmplx(0.0,-dm)
c deposit first time derivative of current
         dcu(1,1,k1) = dcu(1,1,k1) + vx*zt2
         dcu(2,1,k1) = dcu(2,1,k1) + vy*zt2
         dcu(3,1,k1) = dcu(3,1,k1) + vz*zt2
c deposit second time derivative of current
         zt4 = zt4*dm*dm
         dcu2(1,1,k1) = dcu2(1,1,k1) - vx*zt4
         dcu2(2,1,k1) = dcu2(2,1,k1) - vy*zt4
         dcu2(3,1,k1) = dcu2(3,1,k1) - vz*zt4
      else
c deposit first time derivative of current
         zt4 = zt2*cmplx(0.0,dp)
         dcu(1,1,k1) = dcu(1,1,k1) + vx*zt4
         dcu(2,1,k1) = dcu(2,1,k1) + vy*zt4
         dcu(3,1,k1) = dcu(3,1,k1) + vz*zt4
c deposit second time derivative of current
         zt2 = zt2*dp*dp
         dcu2(1,1,k1) = dcu2(1,1,k1) - vx*zt2
         dcu2(2,1,k1) = dcu2(2,1,k1) - vy*zt2
         dcu2(3,1,k1) = dcu2(3,1,k1) - vz*zt2
      endif
   30 continue
!$OMP END PARALLEL DO
c mode number ky = 0
...
c !$OMP PARALLEL DO PRIVATE(j,dkx,dp,zt1,zt3)
      ...
      zt3 = sctx(j-1)
c deposit first time derivative of current
      zt1 = zt3*cmplx(0.0,-dp)
      dcu(1,j,1) = dcu(1,j,1) + vx*zt1
      dcu(2,j,1) = dcu(2,j,1) + vy*zt1
      dcu(3,j,1) = dcu(3,j,1) + vz*zt1
c deposit second time derivative of current
      zt3 = zt3*dp*dp
      dcu2(1,j,1) = dcu2(1,j,1) - vx*zt3
      dcu2(2,j,1) = dcu2(2,j,1) - vy*zt3
      dcu2(3,j,1) = dcu2(3,j,1) - vz*zt3
c special case to match conventional PIC code
      if (nyhm.eq.nyh) then
         dp = dkx*ux + dkv
         zt3 = at1*sctx(j-1)
...
c deposit second time derivative of current
         zt3 = zt3*dp*dp
         dcu2(1,j,k1) = dcu2(1,j,k1) - vx*zt3
         dcu2(2,j,k1) = dcu2(2,j,k1) - vy*zt3
         dcu2(3,j,k1) = dcu2(3,j,k1) - vz*zt3
      endif
   40 continue
c !$OMP END PARALLEL DO
   50 continue
...
c-----------------------------------------------------------------------
      subroutine MRD4JPOST2GL(part,dcu,dcu2,sctx,qm,ci,nop,idimp,nx,ny, 
     1nxhm,nyhm,nxvh,nyv)
c for 2-1/2d code, this subroutine calculates the first two time
c derivatives of the particle current density for force-free particles,
c used to implement high accuracy maxwell solvers
...
c periodic boundaries, parallelization over fourier modes
c OpenMP version
c 118*nxhm*nyhm flops, 1 divides, 1 sqrt per particle
c 24*nxhm*nyhm + 5 loads and stores
c plus (nxhm + nyhm) sines and cosines/particle
c input: all, output: dcu, dcu2, sctx
c first derivative of current density is calculated from the expression:
...
c-----------------------------------------------------------------------
      subroutine A4MAXWEL2GL(exy,bxy,cu,dcu,dcu2,ffc,ci,dt,wf,wm,nx,ny, 
     1nxhm,nyhm,nxvh,nyv)
c this subroutine solves 2-1/2d maxwell's equation in fourier space for
c transverse electric and magnetic fields with periodic boundary
c conditions, using a modified version of the analytic scheme due to
c Irving Haber, extending the constant current approximation originally
c used with a first and second order taylor expansion
c for gridless spectral code.
c input: all, output: wf, wm, exy, bxy
c approximate flop count is: 473*nxhm*nyhm + 169*(nxhm + nyhm)
c plus (3*trig functions + 1 divide)*nxhm*nyhm
c equations being solved are:
c d(c*Bn)/dt = -ick X En and
c En/dt = ick X (c*Bn) - JTn
c Note that the normalization of the E and B fields differ:
c B is normalized to the dimensionless cyclotron frequency.
c solutions are given by:
c En+1 = C*En + iS*(k X (c*Bn)) - S*(JTn+1/2)/c + S*(t/2 - T)*JPn+1/2/c
c -(0.5*S*(t/2)**2/c +  (1 + C)*(t/2 - T)/(k*c)**2)*JPP
c c*Bn+1 = C*(c*Bn) - iS*(k X En) + iS*T*(k X JTn+1/2) + 
c i*((1 + C))*((t/2 - T)*(k X JPn+1/2)/(k*k*c)
c -i*((1 + C)*(t/2 - T)*(t/2)/(k*k*c) + 0.5*S*T*(5/2)**2
c -(t*t/2 - (1 - C)/(k*c)**2))/(k*k*c))*(k X JPP)
c where En = input exy, En+1 = output exy
c Bn = input bxy, Bn+1 = output bxy,
c JTn+1/2 =  affp*cu*s(kx,ky), JPn+1/2 = affp*dcu*s(kx,ky), and
c JPP = affp*dcu2*s(kx,ky)
c C = cos(k*c*dt),  S = sin(k*c*dt)/k, T = tan(k*c*dt/2)/kc
c kx = 2pi*j/nx, ky = 2pi*k/ny, k = sqrt(kx*kx + ky*ky),
c c = 1.0/ci and s(kx,ky) = exp(-((kx*ax)**2+(ky*ay)**2)
c cu(i,j,k) = transverse part of complex current density
c dcu(i,k,j) = transverse part of first derivative of complex current
c density
c dcu2(i,k,j) = transverse part of second derivative of complex current
c density
c exy(i,j,k) = complex transverse electric field
c bxy(i,j,k) = complex magnetic field
c for component i, all for fourier mode (j-1,k-1)
...
c nyv = second dimension of field arrays, must be >= 2*nyhm
      ...
      integer nx, ny, nxhm, nyhm, nxvh, nyv
      real ci, dt, wf, wm
      complex exy, bxy, cu, dcu, dcu2, gxy, hxy, ffc
      dimension exy(3,nxvh,nyv), bxy(3,nxvh,nyv)
      dimension cu(3,nxvh,nyv), dcu(3,nxvh,nyv), dcu2(3,nxvh,nyv)
      dimension gxy(3,nxvh,nyv), hxy(3,nxvh,nyv)
      dimension ffc(nxhm,nyhm)
c local data
      integer ny2, j, k, k1
      real dnx, dny, dth, cc, cdth, affp, anorm, dkx, dky, dky2, t2
      real t, c, s, sc, afs, aft, dfs, dft, dt2, et, tc, efs, eft
      complex zero, zt1, zt2, zt3, zt4, zt5, zt6, zt7, zt8, zt9
      complex ct1, ct2
      double precision wp, ws, swp, sws
      if (ci.le.0.0) return
...
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO
!$OMP& PRIVATE(j,k,k1,dky,dky2,dkx,aft,t2,t,s,c,afs,sc,dft,dfs,et,tc,efs
!$OMP& ,eft,zt1,zt2,zt3,zt4,zt5,zt6,zt7,zt8,zt9,ws,wp)
!$OMP& REDUCTION(+:swp,sws)
      do 20 k = 2, nyhm
      k1 = ny2 - k
      dky = dny*real(k - 1)
      ...
      s = sin(s)*t2
      t = tan(t)*t2
      dft = dble(dth) - dble(t*ci)
      afs = s*aft
      dfs = afs*dft
      et = (1.0 + c)*t2*aft
      tc = t2*ci
      efs = dt2*afs + et*tc*dft
      eft = (dble(4.0*dt2*aft*t2) - dble(afs*t*ci*tc))*tc - dt2*afs*t*ci
     1 - et*dft*dth*tc
      sc = s*cc
      dft = et*tc*dft
      aft = aft*t
...
c calculate iB
      ...
      zt3 = cmplx(-aimag(bxy(1,j,k)),real(bxy(1,j,k)))
c update electric field
      zt4 = c*exy(1,j,k) + sc*(dky*zt1) - afs*cu(1,j,k) + dfs*dcu(1,j,k)
     1 - efs*dcu2(1,j,k)
      zt5 = c*exy(2,j,k) - sc*(dkx*zt1) - afs*cu(2,j,k) + dfs*dcu(2,j,k)
     1 - efs*dcu2(2,j,k)
      zt6 = c*exy(3,j,k) + sc*(dkx*zt2 - dky*zt3) - afs*cu(3,j,k) +     
     1dfs*dcu(3,j,k) - efs*dcu2(3,j,k)
c calculate iE
      zt1 = cmplx(-aimag(exy(3,j,k)),real(exy(3,j,k)))
      zt2 = cmplx(-aimag(exy(2,j,k)),real(exy(2,j,k)))
      zt3 = cmplx(-aimag(exy(1,j,k)),real(exy(1,j,k)))
...
c update magnetic field 
      ...
      zt6 = c*bxy(3,j,k) - s*(dkx*(zt2 - aft*zt8) - dky*(zt3 - aft*zt9))
c calculate jperpp
      zt1 = cmplx(-aimag(dcu(3,j,k)),real(dcu(3,j,k)))
      zt2 = cmplx(-aimag(dcu(2,j,k)),real(dcu(2,j,k)))
      zt3 = cmplx(-aimag(dcu(1,j,k)),real(dcu(1,j,k)))
c calculate jperpp2
      zt7 = cmplx(-aimag(dcu2(3,j,k)),real(dcu2(3,j,k)))
      zt8 = cmplx(-aimag(dcu2(2,j,k)),real(dcu2(2,j,k)))
      zt9 = cmplx(-aimag(dcu2(1,j,k)),real(dcu2(1,j,k)))
c add corrections to magnetic field
      zt4 = zt4 + dft*dky*zt1 + eft*dky*zt7
      zt5 = zt5 - dft*dkx*zt1 - eft*dkx*zt7
      zt6 = zt6 + dft*(dkx*zt2 - dky*zt3) + eft*(dkx*zt8 - dky*zt9)
c store magnetic field and calculate energy
      bxy(1,j,k) = zt4
      bxy(2,j,k) = zt5
      bxy(3,j,k) = zt6
...
c calculate iB
      ...
      zt3 = cmplx(-aimag(bxy(1,j,k1)),real(bxy(1,j,k1)))
c update electric field
      zt4 = c*exy(1,j,k1) - sc*(dky*zt1) - afs*cu(1,j,k1) + dfs*        
     1dcu(1,j,k1) - efs*dcu2(1,j,k1)
      zt5 = c*exy(2,j,k1) - sc*(dkx*zt1) - afs*cu(2,j,k1) + dfs*        
     1dcu(2,j,k1) - efs*dcu2(2,j,k1)
      zt6 = c*exy(3,j,k1) + sc*(dkx*zt2 + dky*zt3) - afs*cu(3,j,k1) +   
     1dfs*dcu(3,j,k1) - efs*dcu2(3,j,k1)
c calculate iE
      zt1 = cmplx(-aimag(exy(3,j,k1)),real(exy(3,j,k1)))
      zt2 = cmplx(-aimag(exy(2,j,k1)),real(exy(2,j,k1)))
      zt3 = cmplx(-aimag(exy(1,j,k1)),real(exy(1,j,k1)))
...
c update magnetic field 
     ...
     1)
c calculate jperpp
      zt1 = cmplx(-aimag(dcu(3,j,k1)),real(dcu(3,j,k1)))
      zt2 = cmplx(-aimag(dcu(2,j,k1)),real(dcu(2,j,k1)))
      zt3 = cmplx(-aimag(dcu(1,j,k1)),real(dcu(1,j,k1)))
c calculate jperpp2
      zt7 = cmplx(-aimag(dcu2(3,j,k1)),real(dcu2(3,j,k1)))
      zt8 = cmplx(-aimag(dcu2(2,j,k1)),real(dcu2(2,j,k1)))
      zt9 = cmplx(-aimag(dcu2(1,j,k1)),real(dcu2(1,j,k1)))
c add corrections to magnetic field
      zt4 = zt4 - dft*dky*zt1 - eft*dky*zt7
      zt5 = zt5 - dft*dkx*zt1 - eft*dkx*zt7
      zt6 = zt6 + dft*(dkx*zt2 + dky*zt3) + eft*(dkx*zt8 + dky*zt9)
...
c calculate iB
      ...
      zt3 = cmplx(-aimag(bxy(1,1,k)),real(bxy(1,1,k)))
c update electric field
      zt4 = c*exy(1,1,k) + sc*(dky*zt1) - afs*cu(1,1,k) + dfs*dcu(1,1,k)
     1 - efs*dcu2(1,1,k)
      zt6 = c*exy(3,1,k) - sc*(dky*zt3) - afs*cu(3,1,k) + dfs*dcu(3,1,k)
     1 - efs*dcu2(3,1,k)
c calculate iE
      zt1 = cmplx(-aimag(exy(3,1,k)),real(exy(3,1,k)))
      zt3 = cmplx(-aimag(exy(1,1,k)),real(exy(1,1,k)))
...
c update magnetic field 
      ...
      zt6 = c*bxy(3,1,k) + s*dky*(zt3 - aft*zt9)
c calculate jperpp
      zt1 = cmplx(-aimag(dcu(3,1,k)),real(dcu(3,1,k)))
      zt3 = cmplx(-aimag(dcu(1,1,k)),real(dcu(1,1,k)))
c calculate jperpp2
      zt7 = cmplx(-aimag(dcu2(3,1,k)),real(dcu2(3,1,k)))
      zt9 = cmplx(-aimag(dcu2(1,1,k)),real(dcu2(1,1,k)))
c add corrections to magnetic field
      zt4 = zt4 + dky*(dft*zt1 + eft*zt7)
      zt6 = zt6 - dky*(dft*zt3 + eft*zt9)
c store magnetic field and calculate energy
      bxy(1,1,k) = zt4
      bxy(2,1,k) = zero
      bxy(3,1,k) = zt6
...
c update electric field
         zt4 = c*exy(1,1,k1) - sc*(dky*zt1) - afs*cu(1,1,k1) +          
     1dfs*dcu(1,1,k1) - efs*dcu2(1,1,k1)
         zt6 = c*exy(3,1,k1) + sc*(dky*zt3) - afs*cu(3,1,k1) +          
     1dfs*dcu(3,1,k1) - efs*dcu2(3,1,k1)
c calculate iE
         zt1 = cmplx(-aimag(exy(3,1,k1)),real(exy(3,1,k1)))
         zt3 = cmplx(-aimag(exy(1,1,k1)),real(exy(1,1,k1)))
c store electric field and calculate energy
...
c update magnetic field 
         ...
         zt6 = c*bxy(3,1,k1) - s*dky*(zt3 - aft*zt9)
c calculate jperpp
         zt1 = cmplx(-aimag(dcu(3,1,k1)),real(dcu(3,1,k1)))
         zt3 = cmplx(-aimag(dcu(1,1,k1)),real(dcu(1,1,k1)))
c calculate jperpp2
         zt7 = cmplx(-aimag(dcu2(3,1,k1)),real(dcu2(3,1,k1)))
         zt9 = cmplx(-aimag(dcu2(1,1,k1)),real(dcu2(1,1,k1)))
c add corrections to magnetic field
         zt4 = zt4 - dky*(dft*zt1 + eft*zt7)
         zt6 = zt6 + dky*(dft*zt3 + eft*zt9)
c store magnetic field and calculate energy
...
c calculate iB
      ...
      zt2 = cmplx(-aimag(bxy(2,j,1)),real(bxy(2,j,1)))
c update electric field
      zt5 = c*exy(2,j,1) - sc*(dkx*zt1) - afs*cu(2,j,1) + dfs*dcu(2,j,1)
     1 - efs*dcu2(2,j,1)
      zt6 = c*exy(3,j,1) + sc*(dkx*zt2) - afs*cu(3,j,1) + dfs*dcu(3,j,1)
     1 - efs*dcu2(3,j,1)
c calculate iE
      zt1 = cmplx(-aimag(exy(3,j,1)),real(exy(3,j,1)))
      zt2 = cmplx(-aimag(exy(2,j,1)),real(exy(2,j,1)))
...
c update magnetic field 
      ...
      zt6 = c*bxy(3,j,1) - s*dkx*(zt2 - aft*zt8)
c calculate jperpp
      zt1 = cmplx(-aimag(dcu(3,j,1)),real(dcu(3,j,1)))
      zt2 = cmplx(-aimag(dcu(2,j,1)),real(dcu(2,j,1)))
c calculate jperpp2
      zt7 = cmplx(-aimag(dcu2(3,j,1)),real(dcu2(3,j,1)))
      zt8 = cmplx(-aimag(dcu2(2,j,1)),real(dcu2(2,j,1)))
c add corrections to magnetic field
      zt5 = zt5 - dkx*(dft*zt1 - eft*zt7)
      zt6 = zt6 + dkx*(dft*zt2 + eft*zt8)
c store magnetic field and calculate energy
      bxy(1,j,1) = zero
      bxy(2,j,1) = zt5
      bxy(3,j,1) = zt6
...
c find electric field
   ...
   10 continue
      ...
      ez = 0.0d0
c mode numbers 0 < kx < nxhm and 0 < ky < nyhm
!$OMP PARALLEL DO PRIVATE(j,k,k1,dky,exl,eyl,ezl,zt1,zt2,zt3,zt4)
...
c-----------------------------------------------------------------------
      subroutine MA0RBPUSH23GL(part,fxy,bxy,qbm,dt,dtc,ci,ek,idimp,nop, 
     1nx,ny,nxhm,nyhm,nxvh,nyv,ipbc)
c for 2-1/2d code, this subroutine updates particle co-ordinates and
c velocities using leap-frog scheme in time using gridless spectral
c version, for relativistic particles with magnetic field, periodic
...
!$OMP& PRIVATE(i,j,k,k1,x,y,dkx,dky,at1,at3,zr3,zi3,zr4,zi4,frp,fip,frm,
!$OMP& fim,exl,eyl,ezl,bxl,byl,bzl,ex,ey,ez,bx,by,bz,dx,dy,dz,ox,oy,oz, 
!$OMP& acx,acy,acz,omxt,omyt,omzt,omt,omti,epl,anorm,rot1,rot2,rot3,
!$OMP& rot4,rot5,rot6,rot7,rot8,rot9,p2,gam,gami,qtmg,dtg,zt1,zt2,zt3,
!$OMP& zt4,ssctx)      
!$OMP& REDUCTION(+:sum1)
      do 50 i = 1, nop
      x = part(1,i)
