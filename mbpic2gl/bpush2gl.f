...
c 60*nxhm*nyhm + 203 flops/particle, 3*nxhm*nyhm+ 5 loads, 5 stores
c 8 divides, 3 sqrts, 2 tans
c plus (nxhm + nyhm) sines and cosines/particle
c input: all, output: part, sctx, ek
c momentum equations used are:
...
c gami = 1/gams, and where the first approximate to gams comes from:
c gams = gam(0) + dg0*tau/w+d2g0*tau**2/6+d3g0*tau**3/24, where
c gam(0) = sqrt(1.0 + p2(0)*ci*ci)
...
c local data
      ...
      real e2, acx, acy, acz, eu, omt, omti, epl, omxt, omyt, omzt, ugt0
      real tau, gams, et2, etu, u2gt0, tn, gami, qtmg, dtg, x, y
...
c dgamma_perp/dtau
      ...
      at1 = -omt*omt*ugt0
c first guess for gams using third order taylor series
      tau = dt/gam0
      gams = gam0 + (0.5*ug0 + sixth*(u2g0 + 0.25*at1*tau)*tau)*tau*ci2
c time-centered kinetic energy
      at1 = -omt*omt*ugt0
      wk = wk + 0.5*(ug0 + 0.25*(u2g0 + sixth*at1*tau)*tau)*tau
      sum1 = sum1 + wk
c optional second guess for gams using integral of t/tau
c
      tau = dt/gams
      u2g0 = gams*e2 + eu
c d2gamma_perp/dtau2
      et2 = dx*dx + dy*dy + dz*dz
      etu = dx*acx + dy*acy + dz*acz
      u2gt0 = gams*et2 + etu
c sin(2x)=2tan(x)/(1+tan(x)**2), cos(2x)-1=-2tan(x)**2/(1+ tan(x)**2)
      tn = tan(0.5*omt*tau)
      at1 = 2.0*tn/(1.0 + tn*tn)
      gams = (0.5*(ug0 - ugt0) + sixth*(u2g0 - u2gt0)*tau)*tau
      ugt0 = omti*ugt0
      u2gt0 = omti*omti*u2gt0
      gams = gam0 + (gams + u2gt0 + (ugt0*tn - u2gt0)*at1*omti/tau)*ci2
c
c set proper time
      gami = 1.0/gams
      qtmg = dth*gami
