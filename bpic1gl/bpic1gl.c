...
/* deposit current with standard procedure: updates part, cue, cux0 */
      ...
      if (relativity==1)
         crdjpost1gl(part,cue,sctx,&cux0,qme,dth,ci,np,idimp,nx,nxhm,
                     nxeh);
      else
         cdjpost1gl(part,cue,sctx,&cux0,qme,dth,np,idimp,nx,nxhm,nxeh);
      dtimer(&dtime,&itime,1);
      time = dtime;
      tdjpost += time;
...
/* deposit time derivative of current: updates dcu */
         if (relativity==1)
            crd2jpost1gl(part,dcu,sctx,qme,ci,np,idimp,nx,nxhm,nxeh);
         else
            cd2jpost1gl(part,dcu,sctx,qme,np,idimp,nx,nxhm,nxeh);
...
/*       } */
/*       if (relativity==1) */
/*          crdsjpost1gl(part,cue,dcu,sctx,qme,ci,np,idimp,nx,nxhm, */
/*                       nxeh);                                     */
/*       else                                                       */
/*          cdsjpost1gl(part,cue,dcu,sctx,qme,ci,np,idimp,nx,nxhm,  */
/*                      nxeh);                                      */
...
/* analytic Boris mover, gamma constant during time step */
         carbpush13gl(part,fxyze,byze,sctx,omx,qbme,dt,dth,ci,&wke,
                      idimp,np,nx,nxhm,nxeh);
/* exact analytic Boris mover, gamma varies during time step */
/*       cearbpush13gl(part,fxyze,byze,sctx,omx,qbme,dt,dth,ci,&wke, */
/*                    idimp,np,nx,nxhm,nxeh);                        */
      else
         cabpush13gl(part,fxyze,byze,sctx,omx,qbme,dt,dth,&wke,idimp,np,
                     nx,nxhm,nxeh);
      dtimer(&dtime,&itime,1);
      time = dtime;
      tpush += time;
