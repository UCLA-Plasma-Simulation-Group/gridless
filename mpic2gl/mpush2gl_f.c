/* C Library for Skeleton 2D Electrostatic Gridless OpenMP PIC Code */
...
void mpush2gl_(double *part, double complex *fxy, double *qbm,
               double *dt, double *ek, int *idimp, int *nop, int *nx,
               int *ny, int *nxhm, int *nyhm, int *nxvh, int *nyv,
               int *ipbc);

void m1push2gl_(double *part, double complex *fxy, double complex *sctx,
...
void mevfield22gl_(double complex *fxy, double *gxy,
...
/*--------------------------------------------------------------------*/
void cmpush2gl(double part[], double complex fxy[], double qbm,
               double dt, double *ek, int idimp, int nop, int nx,
               int ny, int nxhm, int nyhm, int nxvh, int nyv,
               int ipbc) {
   mpush2gl_(part,fxy,&qbm,&dt,ek,&idimp,&nop,&nx,&ny,&nxhm,&nyhm,&nxvh,
             &nyv,&ipbc);
   return;
}
...
void cm1push2gl(double part[], double complex fxy[],
   ...
   m1push2gl_(part,fxy,sctx,&qbm,&dt,ek,&idimp,&nop,&nx,&ny,&nxhm,&nyhm,
              &nxvh,&nyv,&ipbc);
   return;
}
...
void cmevfield22gl(double complex fxy[], double gxy[],
                   ...
                   int ny, int nxhm, int nyhm, int nxvh, int nyv) {
   mevfield22gl_(fxy,gxy,sctx,&xp,&yp,&nx,&ny,&nxhm,&nyhm,&nxvh,&nyv);
   return;
}
