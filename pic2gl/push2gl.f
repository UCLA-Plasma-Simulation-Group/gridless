c Fortran Library for Skeleton 2D Electrostatic Gridless PIC Code
...
c-----------------------------------------------------------------------
      subroutine EVFIELD22GL(fxy,gxy,sctx,xp,yp,nx,ny,nxhm,nyhm,nxvh,nyv
     1)
...
c if nxhm=nx/2 and nyhm=ny/2, nx/2 and ny/2 modes are included, in a
c format compatible with the real to complex FFT used in traditional PIC
c gxy(1) = x component of force/charge at location (xp,yp)
c gxy(2) = y component of force/charge at location (xp,yp)
c sctx = scratch array for sines and cosines
c xp/yp = position to evaluate field
c nx/ny = system length in x/y direction
c nxhm/nyhm = number of fourier modes kept in x/y
c nxvh = first dimension of field, must be >= nxhm
c nyv = second dimension of field, must be >= 2*nyhm
      implicit none
...
c-----------------------------------------------------------------------
      subroutine FIELD22GL(fxy,gxy,sctx,nx,ny,nxhm,nyhm,nxvh,nyv)
c for 2d code, this subroutine calculates real space 2d field gxy at
